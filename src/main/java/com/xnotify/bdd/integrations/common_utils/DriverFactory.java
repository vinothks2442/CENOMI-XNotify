...
public class DriverFactory {
	...
	public final void setMobileDriver(
			String platform,
			String udid,
			String systemPort,
			String deviceName)
		...
		if (strMobileExecutiontype.contains("ios")) {
			...
			capabilities.setCapability(
					MobileCapabilityType.SUPPORTS_LOCATION_CONTEXT,
					true);

			capabilities.setCapability(
					"app",
					System.getProperty("user.dir")
							+ ConfigReader.getValue("iOSAppLocation"));

			URL url = new URL("http://127.0.0.1:4723");

			appiumDriver.set(
					new AppiumDriver(url, capabilities));
		...
		// =========================
			...
			capabilities.setCapability(
					...
					System.getProperty("user.dir")
							+ ConfigReader.getValue("AppLocation"));
			...
			try {
				...
				System.out.println(
						"FAILED TO INITIALIZE MOBILE DRIVER");

				e.printStackTrace();

				throw new RuntimeException(
						"Mobile Driver initialization failed",
						e);
			}
		...
		// =========================
		...
		else if (strMobileExecutiontype
				.equalsIgnoreCase("pCloudy_android")) {
			...
			capabilities.setCapability(
					"appActivity",
					"in.gypsee.customer.activities.SplashActivity");
